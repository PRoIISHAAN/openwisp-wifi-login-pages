...
describe("<Registration /> interactions", () => {
  ...
  it("should execute handleSubmit correctly when form is submitted", async () => {
    ...
    fireEvent.change(password1Input, {target: {value: "wrong password", name: "password1"}});
    fireEvent.change(password2Input, {target: {value: "wrong password1", name: "password2"}});
    fireEvent.submit(form);
    
    await waitFor(() => {
      expect(container.querySelector(".row.password-confirm div.error")).toBeInTheDocument();
      expect(container.querySelector(".row.password-confirm input.error")).toBeInTheDocument();
    });
    
    // Test 2: Matching passwords, API field errors
    fireEvent.change(password1Input, {target: {value: "password", name: "password1"}});
    fireEvent.change(password2Input, {target: {value: "password", name: "password2"}});
    fireEvent.submit(form);
  ...
  it("should show modal if user is already registered with other organizations", async () => {
    ...
    fireEvent.change(emailInput, {target: {value: "tester@openwisp.org", name: "email"}});
    fireEvent.change(password1Input, {target: {value: "testpassword", name: "password1"}});
    fireEvent.change(password2Input, {target: {value: "testpassword", name: "password2"}});
    fireEvent.submit(form);
...
describe("Registration and Mobile Phone Verification interactions", () => {
  ...
  beforeEach(() => {
    ...
    props.configuration.settings = {...props.configuration.settings, mobile_phone_verification: true};
  });
  ...
  it("should process successfully", async () => {
    ...
    fireEvent.change(phoneInput, {target: {value: "+393660011333", name: "phone_number"}});
    fireEvent.change(emailInput, {target: {value: "tester@openwisp.io", name: "email"}});
    fireEvent.change(password1Input, {target: {value: "tester123", name: "password1"}});
    fireEvent.change(password2Input, {target: {value: "tester123", name: "password2"}});
    fireEvent.submit(form);
  ...
  it("should render modal", () => {
    props = createTestProps();
    const {container} = renderWithProviders(<Routes><Route path="*" element={<Registration {...props} />} />
        </Routes>);
  ...
  it("should send post data with optional fields", async () => {
    ...
    if (firstNameInput) {
      fireEvent.change(firstNameInput, {target: {value: "OpenWISP", name: "first_name"}});
    }
...
describe("Registration without identity verification (Email registration)", () => {
  ...
  it("should not show phone number field", async () => {
    const {container} = mountComponent(props);
    expect(container.querySelector("input[name='phone_number']")).not.toBeInTheDocument();
  });
  ...
  it("should set country when selectedCountry is executed", async () => {
    ...
    // Test country selection if relevant UI exists
